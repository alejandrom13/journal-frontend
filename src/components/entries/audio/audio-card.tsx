...
const AudioCard = ({ entry, index }: any) => {
  ...
  return (
    <motion.div
      className="w-full bg-white/40 p-4 rounded-3xl ease-in group hover:bg-white/50 transition-all duration-150"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
    ...
    >
      <div className="flex flex-row items-center">
        ...
        <div className="text-sm font-semibold text-[#3379E3] pl-2">
          Voice Note
        </div>

        <div className="ml-auto flex flex-row h-full items-center gap-2">
          ...
          <div className="bg-primary/10 rounded-full p-1 text-sm px-4 text-primary">
            {isPlaying
      ...
      <div className="w-full bg-white/50 rounded-xl flex flex-row items-center gap-2 p-2">
        <Button
          onClick={onPlayPause}
          className="rounded-full w-[42px] h-[40px]"
          size={"icon"}
        >
